  ...
  <div class="container" style="padding-top:2rem; padding-bottom:4rem;">
    ...
    <div class="d-flex justify-between align-center mb-4 animate-in">
      ...
      <c:choose>
        ...
        <c:when test="${not empty queuedRide}">
          <span class="btn btn-primary" style="opacity:0.6; cursor:not-allowed;">
            <i class="bi bi-hourglass-split"></i> Queued #${queuePosition}
          </span>
        </c:when>
        <c:otherwise>
    ...
    <%-- ── Queued Ride Banner (waiting in queue) ─────────────────────── --%>
    <c:if test="${not empty queuedRide}">
    <div class="card animate-in-1" style="border-color:#f59e0b; margin-bottom:1.5rem; background:rgba(245,158,11,0.05);">
      <div class="d-flex justify-between align-center">
        <div class="d-flex align-center gap-2">
          <div style="font-size:2rem;">⏳</div>
          <div>
            <div class="fw-bold" style="color:#f59e0b;">Queued — Waiting for Bike</div>
            <div class="text-muted" style="font-size:0.85rem;">
              Ride ID: ${queuedRide.rideId} &nbsp;|&nbsp; Requested: ${queuedRide.startTime}
            </div>
            <div style="font-size:0.85rem; margin-top:0.3rem;">
              <span style="background:rgba(245,158,11,0.15); color:#f59e0b; padding:2px 10px; border-radius:20px; font-weight:600;">
                Position #${queuePosition} of ${queueSize} in queue
              </span>
              &nbsp; You will be activated automatically when the bike becomes available.
            </div>
          </div>
        </div>
        <div>
          <form action="/rides/cancel" method="post" style="display:inline;">
            <input type="hidden" name="rideId" value="${queuedRide.rideId}"/>
            <button type="submit" class="btn btn-danger btn-sm"
                    data-confirm="Leave the queue and cancel this ride request?">Leave Queue</button>
          </form>
        </div>
      </div>
    </div>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        <c:choose>
                          <c:when test="${ride.status == 'ACTIVE'}"><span class="badge badge-active"><span class="live-dot" style="width:5px;height:5px;margin-right:3px;"></span>Active</span></c:when>
                      ...
                      <td>
                        <c:choose>
                          <c:when test="${ride.status == 'COMPLETED' && !reviewedRideIds.contains(ride.rideId)}">
                            <a href="/feedback/submit/${ride.rideId}" class="btn btn-ghost btn-sm">Review</a>
                          </c:when>
                          <c:when test="${ride.status == 'COMPLETED' && reviewedRideIds.contains(ride.rideId)}">
                            <span class="badge badge-available" style="font-size:0.72rem;">✅ Reviewed</span>
                          </c:when>
                        </c:choose>
                      </td>
                    </tr>
              ...
              <div class="ride-item mb-2">
                ...
                <div class="d-flex gap-1">
                  ...
                  <form action="/feedback/delete" method="post" style="display:inline;">
                    ...
                    <button type="submit" class="btn btn-danger btn-sm"
                            data-confirm="Delete your review for ride ${fb.rideId}? You can submit a new one after.">
