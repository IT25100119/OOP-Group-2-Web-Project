...
<div class="container" style="padding-top:3rem; padding-bottom:4rem; max-width:560px; text-align:center;">
  ...
  <c:choose>
    <c:when test="${ride.status == 'QUEUED'}">
      <div class="success-ring animate-in" style="background:rgba(245,158,11,0.1); border-color:#f59e0b;">⏳</div>
      <h2 class="animate-in-1" style="color:#f59e0b;">Added to Queue!</h2>
      <p class="text-muted animate-in-2">The bike you requested is currently in use. You've been added to the FIFO queue and will be activated automatically when the bike becomes available.</p>
    </c:when>
    <c:otherwise>
  ...
  <div class="card animate-in-3" style="margin:2rem 0; text-align:left;">
    <div style="display:grid; gap:0.75rem;">
      ...
      <div class="d-flex justify-between">
        ...
        <c:choose>
          <c:when test="${ride.status == 'QUEUED'}">
            <span class="badge badge-maintenance">⏳ Queued — Waiting</span>
          </c:when>
          <c:otherwise>
      ...
      <c:if test="${not empty ride.packageId}">
      <div class="d-flex justify-between">
        <span class="text-muted">Package</span>
        <code class="text-accent">${ride.packageId}</code>
      </div>
      </c:if>
      <c:if test="${empty ride.packageId}">
      <div class="d-flex justify-between">
        <span class="text-muted">Billing</span>
        <span>Pay per hour on return</span>
      </div>
      </c:if>
    </div>
  </div>
