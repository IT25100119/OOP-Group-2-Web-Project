...
public class RideController {
    ...
    public String queueDashboard(HttpSession session, Model model) {
        ...
        model.addAttribute("activeCount", rideService.countActiveRides());
        return "rides/queue";
    }
    ...
    public String rentPage(HttpSession session, Model model) {
        ...
        // Block if user has an active OR already-queued ride
        if (rideService.getActiveRideByUser(user.getUserId()) != null ||
                rideService.getQueuedRideByUser(user.getUserId()) != null) {
        ...
        java.util.Map<String, java.util.List<RentalPackage>> pkgMap = new java.util.HashMap<>();
        for (RentalPackage pkg : packageService.getAllPackages()) {
    ...
    public String requestRide(@RequestParam String bikeId,
        ...
        var ride = rideService.requestRide(user.getUserId(), bikeId, startStationId, packageId);
        if (ride != null) {
